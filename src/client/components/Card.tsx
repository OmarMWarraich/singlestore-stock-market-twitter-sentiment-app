...
const Card: React.FC<CardProps> = ({ investment, setPnls }) => {
  ...
  const { symbol, displayName, priceBought, numberOfShares, currency } =
    investment;
  ...
  const stockPnl =
    stock &&
    (
      stock["05. price"] * numberOfShares -
      priceBought * numberOfShares
    ).toFixed(2);
  const pnlPercentageChange =
    stock &&
    (((stock["05. price"] - priceBought) / priceBought) * 100).toFixed(2);
        ...
        <div className="card-info">
          <div className="symbol-info">
            ...
            <div className="info-container">
              ...
              <p>
                {numberOfShares} {symbol} • {priceBought}
                {currency}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <h3 className={getDirection(stockPnl)}>
              {stockPnl}
              {currency}
            </h3>
            {/*change since you bought the stock*/}
            <p className={getDirection(pnlPercentageChange)}>
              {pnlPercentageChange}%
            </p>
          </div>
        </div>
